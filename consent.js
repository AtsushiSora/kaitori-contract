...
function collectRemoteSeller() {
  ...
  return {
    ...
    sellerWorkplace: sellerInputValue("remote-seller-workplace"),
    sellerWorkplacePhone: sellerInputValue("remote-seller-workplace-phone"),
...
function populateRemoteSeller(data) {
  const mappings = {
    ...
    "remote-seller-workplace": data.sellerWorkplace || data.workplace,
    "remote-seller-workplace-phone": data.sellerWorkplacePhone,
...
function completedCustomerContract(contract, result) {
  const seller = result.seller || contract.data || {};
...
async function completeConsent() {
  ...
  if (loadedContract.cloudMode && window.OrderAutoCloud?.isConfigured()) {
    try {
      completeButton.textContent = "契約書PDFを作成しています";
